...
extends Necrolab {
    public static function saveFromLeaderboardEntry(array $leaderboard_entry, $cache) {
        if(!empty($leaderboard_entry)) {
            ...
            $cache->hSetNx(CacheNames::getPowerRankingModesName($release_id), $mode_id, $mode_id);
        
            $power_ranking_entry_hash_name = CacheNames::getPowerRankingEntryName($release_id, $mode_id, $steam_user_id);
            ...
            $cache->zIncrBy(CacheNames::getCharacterPointsName($release_id, $mode_id, $character_column_prefix), $rank_points, $steam_user_id);
            $cache->zIncrBy(CacheNames::getPowerTotalPointsName($release_id, $mode_id), $rank_points, $steam_user_id);
            
            if($leaderboard_entry['is_deathless'] == 1) {
                $cache->zIncrBy(CacheNames::getDeathlessPointsName($release_id, $mode_id), $rank_points, $steam_user_id);
            }
            elseif($leaderboard_entry['is_score_run'] == 1) {
                $cache->zIncrBy(CacheNames::getScorePointsName($release_id, $mode_id), $rank_points, $steam_user_id);
            }
            elseif($leaderboard_entry['is_speedrun'] == 1) {
                $cache->zIncrBy(CacheNames::getSpeedPointsName($release_id, $mode_id), $rank_points, $steam_user_id);
            }
